...
func _ready() -> void:
	_set_speed(10, _player_data.max_velocity)

func _physics_process(_delta: float) -> void:
	_set_speed(10, _player_data.max_velocity)
	_speedometr_arrow_texture_rect.rotation_degrees = _map_speed_to_rot_angle()  + randf_range(-3.0, 2.0)
	_speed_label.text = str(_player_data.current_velocity) + " km/h"
	max_speed_label.text = str(_player_data.max_velocity) + " km/h max"
...
func _map_speed_to_rot_angle() -> float:
	...
	return clamp(linear_derivatve * _player_data.current_velocity - 174, -174, -28)
